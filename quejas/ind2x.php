<?php
	...
	if ($idR=="") {
		echo "<script type='text/javascript'>";
			echo "alert('id denuncia no disponible');";
			echo "var pagina='../quejas';";
			echo "document.location.href=pagina;";
		echo "</script>";
	}
	else{
		$datos="SELECT * from denuncia where id_dn=$idR";
		$sql_dato=mysql_query($datos,$conexion) or die (mysql_error());
		$numdato=mysql_num_rows($sql_dato);
		if ($numdato>0) {
			while ($dt=mysql_fetch_array($sql_dato)) {
				$ttdn=$dt['tit_dn'];
				$rsdn=$dt['res_dn'];
				$xtdn=$dt['txt_dn'];
				$fedn=$dt['fe_dn'];
...
<head>
	...
	<meta name="description" content="<?php echo $rsdn ?>" />
	<meta name="keywords" content="Quejas u opiniones" />
...
<body>
	...
	<section>
		...
		<article id="automargen">
			<i><?php echo fechatraducearray($fedn); ?></i>
		</article>
	</section>
	<footer>
...
<?php
			...
			echo "<script type='text/javascript'>";
				echo "alert('Denuncia no existe o ha sido eliminada');";
				echo "var pagina='../quejas';";
				echo "document.location.href=pagina;";
			echo "</script>";
		}
